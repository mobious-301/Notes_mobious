...
#endif
	finalColor += finalColor * _BrightnessAddtive
	finalColor += emission;
...
half3 ProcessLightLambert(InputData inputData, half4 shadowMask, uint meshRenderingLayers, int lightIndex)
{
	...
	half3 attenuatedLightColor = light.color * saturate(attenuation + _ShadowAddtive);
...
float CalculateDepthFade(float4 screenPos, float fadeDistance)
{
	...
	float sceneDepth = LinearEyeDepth(GetSceneDepth(positionNorm.xy+float2(-0.02*(1-saturate(fadeDistance)),0.005)), _ZBufferParams);
	// float surfaceDepth = LinearEyeDepth(positionNorm.z, _ZBufferParams);
	float surfaceDepth = LinearEyeDepth(GetSceneDepth(positionNorm.xy), _ZBufferParams);
...
#endif
	pixel.xyz +=pixel.xyz*_BrightnessAddtive;





	float distance = length(input.screenPos)/65;


	//颜色混合 颜色混合
	float depthDiff =saturate( pow(CalculateDepthFade( input.screenPos,distance),2));

	float3 lightColor = lerp(float3(1,1,1) , texureColor,0.3);

	pixel.xyz = lerp(pixel.xyz , lightColor,depthDiff);

	// 颜色混合b 正片叠底

	// float3 lightColor = lerp(float3(0,0,0) , float3(1,1,1),depthDiff);

	// pixel.xyz = pixel.xyz * lightColor;
