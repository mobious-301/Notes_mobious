Shader "Custom/URPRainEffectPointBillRandom"
{
    Properties
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            RainPoint ProcessRainPoint(uint instanceID)
            {
                ...
                float tempHeight =  clamp (result.height , _StopHeight,1)+ _RainPosOffset.z;


                // 4. 设置世界空间位置
                result.position = float3(
                pos.x,
                tempHeight * _RainHeight,
                pos.z
                );
            ...
            float3 windOS(float3 worldPos, float random){
                ...
                float3 windDir = normalize(_WindDirection);
                float windStrength = _WindStrength * (1 + sin(_Time.y * _WindSpeed));
                float3 wind = windDir * windStrength * (_RainHeight - worldPos.y * worldPos.y * 0.2) * random; //worldPos
