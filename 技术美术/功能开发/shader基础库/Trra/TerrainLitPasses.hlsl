                  ...
                  NormalWS normalWS)
{
    ...
    nrm += splatControl.r * NorBlendWSC(normalWS.positionWS, normalWS.triWeights, _Normal0, _Splat0_ST, sampler_Normal0,
                                        _NormalScale0);
    nrm += splatControl.g * NorBlendWSC(normalWS.positionWS, normalWS.triWeights, _Normal1, _Splat1_ST, sampler_Normal0,
                                        _NormalScale1);
    nrm += splatControl.b * NorBlendWSC(normalWS.positionWS, normalWS.triWeights, _Normal2, _Splat2_ST, sampler_Normal0,
                                        _NormalScale2);
    nrm += splatControl.a * NorBlendWSC(normalWS.positionWS, normalWS.triWeights, _Normal3, _Splat3_ST, sampler_Normal0,
                                        _NormalScale3);
...
void SplatmapMix(float4 uvMainAndLM, float4 uvSplat01, float4 uvSplat23, inout half4 splatControl, out half weight,
                 out half4 mixedDiffuse, out half4 defaultSmoothness, inout half3 mixedNormal, NormalWS normalWS)
{
    half4 diffAlbedo[4];
    #if defined(_NORMALMAP)
    diffAlbedo[0] = TexBlendWS(normalWS.positionWS, normalWS.triWeights, _Splat0, _Splat0_ST, sampler_Splat0);
    diffAlbedo[1] = TexBlendWS(normalWS.positionWS, normalWS.triWeights, _Splat1, _Splat1_ST, sampler_Splat0);
    diffAlbedo[2] = TexBlendWS(normalWS.positionWS, normalWS.triWeights, _Splat2, _Splat2_ST, sampler_Splat0);
    diffAlbedo[3] = TexBlendWS(normalWS.positionWS, normalWS.triWeights, _Splat3, _Splat3_ST, sampler_Splat0);
    #else
    ...
    if (_NumLayersCount <= 4)
    {
        ...
        half4 opacityAsDensity = saturate(
            (half4(diffAlbedo[0].a, diffAlbedo[1].a, diffAlbedo[2].a,
                   diffAlbedo[3].a) - (1 - splatControl)) * 20.0);
    ...
    NormalMapMix(uvSplat01, uvSplat23, splatControl, mixedNormal, normalWS);
}
...
#endif
{
    ...
    // splatControl.xyz = float3(pow(splatControl.y, 0.5), pow(splatControl.z, 0.5), 0); //## 材质有接缝  pow 0.5提量

    // splatControl.y = splatControl.y;
    // splatControl.x = 1 - splatControl.y;
    ...
    NormalWS normalWS;
    #if defined(_NORMALMAP)
    normalWS.worldNormal = IN.normal;
    normalWS.triWeights = GetTriWeightsC(IN.normal);
    normalWS.positionWS = IN.positionWS;
    #endif
    ...
    #else
    occlusion = 1;
    half4 color = UniversalFragmentPBR(inputData, albedo, metallic, /* specular */ half3(0.0h, 0.0h, 0.0h),
                                       smoothness,
                                       occlusion, /* emission */ half3(0, 0, 0), alpha);

    SplatmapFinalColor(color, inputData.fogCoord);
    // color.rgb = splatControl.xyz * alpha;
