Shader "Custom/SphereGroundGlowPlus"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float CalculateDepthFade(float4 screenPos)
            {
                ...
                // return abs((sceneDepth - 1-surfaceDepth) / fadeDistance);  //### 不要用 abs
            ...
            float4 frag(Varyings input, bool isFrontFace : SV_IsFrontFace) : SV_Target
            {
                ...
                float baseShap =  CullArea*(1-CullAreaLerp); // 除以20用于可视化
                ...
                float Noise01=  ComputeWeightedNoise(input.positionWS,normalWeights, _Noise01Tiling,_Noise01ScrollSpeed,_NoiseTexture,_Time.y,0);
                float NoiseShap= saturate(baseShap*Noise01+baseShap*_NoiseMaskAdd);

                float MaskAppear= abs(input.positionOS.y+_MaskAppearLocalYAdd)+_MaskAppearProgress;
